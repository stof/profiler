...
if (class_exists('\\Okvee\\Number\\Number')) {
    $number = new \Okvee\Number\Number();
}
?>

<script>
<?php okvprofilerLoadCss(); ?> 

<?php 
echo 'if (typeof jQuery == \'undefined\') {'."\n";
okvprofilerLoadJs('jquery');
echo '}'."\n\n";
okvprofilerLoadJs(); 
?> 
</script>
<div class="okvprofiler">
    <div class="okvprofiler-container">
        <ul class="okvprofiler-log-sections">
            <li><strong><a href="http://okvee.net" target="okvee_net" class="highlight">Okvee</a></strong>\Profiler</li>
            ...
            if (is_array($this->log_sections)) {
                foreach ($this->log_sections as $section => $data_array) {
                    ...
                    // li of each section.
                    echo '<li class="okvprofiler-see-details">'."\n";
                    ...
                    if ($section == 'Files' && isset($data_array['total_size'])) {
                        echo "\t\t".'<li class="okvprofiler-log-sumary-row">'."\n";
                        echo "\t\t\t".'<div class="okvprofiler-log-file-totalsize">Total size</div>'."\n";
                        echo "\t\t\t".'<div class="okvprofiler-log-file-totalsize-value">';
                        if (isset($number)) {
                    ...
                    if ($section == 'Files' && isset($data_array['largest_size'])) {
                        echo "\t\t".'<li class="okvprofiler-log-sumary-row">'."\n";
                        echo "\t\t\t".'<div class="okvprofiler-log-file-largestsize">Largest size</div>'."\n";
                        echo "\t\t\t".'<div class="okvprofiler-log-file-largestsize-value">';
                        if (isset($number)) {
                    ...
                    if (is_array($data_array) && !empty($data_array)) {
                        ...
                        foreach ($data_array as $data_key => $data_values) {
                            if (is_numeric($data_key) && is_array($data_values) && array_key_exists('data', $data_values)) {
                                ...
                                if (isset($data_values['logtype'])) {
                                    echo "\t\t\t".'<div class="okvprofiler-log-logtype '.strip_tags($data_values['logtype']).'">'.strip_tags(ucfirst($data_values['logtype'])).'</div>'."\n";
                                }

                                echo "\t\t\t".'<pre class="okvprofiler-log-data">'.  htmlspecialchars(trim(print_r($data_values['data'], true))).'</pre>'."\n";

                                if ((isset($data_values['file']) && $data_values['file'] != null) || (isset($data_values['line']) && $data_values['line'] != null)) {
                                    echo "\t\t\t";
                                    echo '<div class="okvprofiler-log-fileline">';
                                ...
                                if ($section == 'Time Load' && isset($data_values['time'])) {
                                    echo "\t\t\t".'<div class="okvprofiler-log-time">'.$this->getReadableTime(($data_values['time']-$this->start_time)*1000).'</div>'."\n";
                                }
                                ...
                                if ($section == 'Files' && isset($data_values['size'])) {
                                    echo "\t\t\t".'<div class="okvprofiler-log-filesize">';
                                    if (isset($number)) {
                                ...
                                if (($section == 'Get' || $section == 'Post' || $section == 'Session') && isset($data_values['inputvalue'])) {
                                    echo "\t\t\t".'<pre class="okvprofiler-log-inputs-value">'.htmlspecialchars(print_r($data_values['inputvalue'], true)).'</pre>';
                                }
                        ...
                        echo "\t\t".'<li><pre class="okvprofiler-log-data">There is no data to display.</pre></li>'."\n";
                    }
            ...
            ?> 
        </ul><!--.okvprofiler-log-sections-->
    </div><!--.okvprofiler-container-->
</div><!--.okvprofiler-->
<?php
unset($number);